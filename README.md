...
3. **准备知识库数据**
   - 将文本内容保存在 `public/example-structuredDATA/content.json`
   - 将图片元数据保存在 `public/example-structuredDATA/image.json`
...
1. **准备自定义数据**
   - 创建内容JSON文件，格式参考 `example-structuredDATA/content.json`
   - 创建图片元数据JSON文件，格式参考 `example-structuredDATA/image.json`
   - 准备图片文件
